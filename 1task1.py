...
def list_ifname_ip(file_name):
	...
	for i in range(len(list1)):
		if list1[i]=='interface':
			...
			if list1[i]=='no' and list1[i+1]=='nameif':
				list3.append('no name')
				list4.append('no vlan')	#FOR THE INTERFACE WHICH HAS NO NAME , NO IP  AND NO SUBNETMASK
				lst5.append('no ip ')
				...
				if list1[i-1]=='management-only':	#  NETWORK CAS
	...
	for i in range(len(list2)):
		...
		lst7.append(lst5[i])
		lst7.append(lst6[i])
		dict[list2[i]]=lst7	#add  nameif,ip,netMask as value in dict
	return dict
